...
3. 解压 `momenttheatre-1.0.0.zip`，或者打开同名发布目录；上传**里面的文件和文件夹**，不要上传 ZIP，也不要上传外面的 momenttheatre-1.0.0 文件夹。
...
无需开启 GitHub Pages；酒馆安装的是 Git 仓库，不是托管网页。可以额外创建 `v1.0.0` Release 展示更新说明，安装和更新依然来自默认分支中的扩展文件。

后续版本更新同一仓库的文件，保持入口和目录结构，提升 manifest 版本号并更新 CHANGELOG。用户在酒馆扩展管理中更新并刷新，不必卸载。

本次只准备本地发布文件，尚未创建仓库或上传。仓库尚无实际地址，因此没有把虚构的用户名写入扩展配置；「检查更新」从酒馆当前 Git 安装目录读取真实远程仓库，无需预填账号。
