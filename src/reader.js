...
import { renderProseInline, PROSE_CSS } from './prose.js';
import { compileThemeCss } from './theme-css.js';
export const escapeHtml = value => String(value ?? '').replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
export const PHONE_TYPES = ['text', 'voice', 'transfer', 'image', 'sticker', 'location', 'share', 'call', 'video'];
export function parsePhone(content) {
  const cleaned = String(content).trim().replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/, '');
  let data;
  try { data = JSON.parse(cleaned); } catch { throw new Error('小手机内容不是有效 JSON，可以编辑原文修正；生成内容已保留。'); }
  const messages = Array.isArray(data) ? data : data?.messages;
  if (!Array.isArray(messages) || messages.length > 2000) throw new Error('小手机需要 messages 消息数组，最多 2000 条。');
  const normalized = []; let legacyTime = '';
  for (const [index, m] of messages.entries()) {
    if (!m || typeof m !== 'object' || !['user', 'char', 'system'].includes(m.sender)) throw new Error(`第 ${index + 1} 条消息的 type 或 sender 不正确。`);
    // Old standalone time separators become metadata, never counted as a message.
    if (m.type === 'time') { legacyTime = typeof m.text === 'string' ? m.text.slice(0, 200) : ''; continue; }
    if (!PHONE_TYPES.includes(m.type)) throw new Error(`第 ${index + 1} 条消息的 type 或 sender 不正确。`);
    const item = Object.fromEntries(['type', 'sender', 'time', 'text', 'duration', 'amount', 'status', 'url', 'title', 'description', 'source', 'thumbnail', 'address', 'sticker'].filter(key => typeof m[key] === 'string' && m[key]).map(key => [key, m[key].slice(0, 20000)]));
    if (!item.time && legacyTime) item.time = legacyTime;
    normalized.push(item);
  }
  return normalized;
}
export function safeImage(value) {
  try { const u = new URL(value); return u.protocol === 'https:' || /^data:image\/(png|jpeg|gif|webp);base64,/i.test(value) ? value : ''; } catch { return ''; }
}
function phoneHtml(messages, characterName) {
  ...
  return `<div class="phone"><div class="phone-screen"><div class="phone-status"><span>${escapeHtml(clock)}</span><span class="phone-island" aria-hidden="true"></span><span class="phone-battery" aria-hidden="true"><i></i></span></div><div class="phone-header" style="display:grid!important;grid-template-columns:24px minmax(0,1fr) 24px!important;column-gap:8px!important;padding-inline:17px!important;direction:ltr!important"><span class="phone-back" aria-hidden="true" style="grid-column:1!important;grid-row:1!important;text-align:left!important">‹</span><div class="phone-contact" style="grid-column:2!important;grid-row:1!important;text-align:center!important;min-width:0!important;width:100%!important;margin-inline:0!important;position:static!important;transform:none!important" title="${escapeHtml(characterName)}">${escapeHtml(characterName)}</div></div><div class="messages" tabindex="0" role="region" aria-label="手机消息，可在屏幕内滚动">${messages.map(m => {
    ...
    if (m.type === 'transfer') inner = `<div class="transfer-card"><div class="transfer-main">${phoneIcon('transfer')}<div><strong>${escapeHtml(m.amount)}</strong><span>${escapeHtml(m.status || '待收款')}</span></div></div>${body ? `<p class="transfer-note">${body}</p>` : ''}<div class="transfer-foot">转账</div></div>`;
    if (m.type === 'image') {
    ...
    if (['call', 'video'].includes(m.type)) inner = `<span class="call-record">${phoneIcon(m.type)}<span>发起${m.type === 'call' ? '语音' : '视频'}通话</span></span>`;
...
export function renderReader(story, theme, chapterIndex = null, themeForChapter = null) {
  ...
  const body = chapters.map((chapter, index) => {
    ...
    let content;
    if (mode === 'phone') {
      try { content = phoneHtml(parsePhone(chapter.content), story.snapshot?.character?.name || '角色'); }
      catch (error) { content = `<p class="diagnostic">${escapeHtml(error.message)}</p><pre>${escapeHtml(chapter.content)}</pre>`; }
    } else {
...
${PHONE_CSS}
${PROSE_CSS}
</style></head><body class="${phoneOnly ? 'phone-reader' : ''}"><main>${firstProse < 0 ? `<h1>${escapeHtml(story.title)}</h1>` : ''}${body || '<p>尚无内容</p>'}</main></body></html>`;
}
