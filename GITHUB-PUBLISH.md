...
3. 解压 `momenttheatre-1.0.8.zip`，或者打开同名发布目录；上传**里面的文件和文件夹**，不要上传 ZIP，也不要上传外面的 momenttheatre-1.0.8 文件夹。
...
无需开启 GitHub Pages；酒馆安装的是 Git 仓库，不是托管网页。可以额外创建 `v1.0.8` Release 展示更新说明，安装和更新依然来自默认分支中的扩展文件。
